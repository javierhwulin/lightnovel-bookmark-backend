...
def _handle_scraper_exception(e: Exception, url: str):
    ...
    if isinstance(e, NovelUpdatesScraperError):
        raise HTTPException(
            status_code=400, detail=f"Failed to scrape from {url}: {str(e)}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Unexpected error while processing {url}: {str(e)}"
        )
...
def preview_novel(request: ScrapeRequest) -> PreviewResponse:
    ...
    if not validate_url(request.url):
        raise HTTPException(
            status_code=400, detail="Invalid URL. Only NovelUpdates URLs are supported."
        )
...
def update_novel_from_source(
    novel_id: int, request: ScrapeRequest, db: Session = Depends(get_db)
) -> LightNovel:
    """
