...
@router.get("/user-preferences/{novel_id}", response_model=Optional[schemas.UserPreferences])
...
@router.post("/user-preferences/{novel_id}", response_model=schemas.UserPreferences, status_code=201)
def create_or_update_user_preferences(
    ...
    """
    try:
        preferences = service.create_or_update_user_preferences(db, novel_id, preferences_data)
        return preferences
    except LightNovelBookmarksException as e:
        _handle_service_exception(e)
...
@router.patch("/user-preferences/{novel_id}/status", response_model=schemas.UserPreferences)
...
@router.patch("/user-preferences/{novel_id}/progress", response_model=schemas.UserPreferences)
...
@router.post("/reading-sessions/{novel_id}/start", response_model=schemas.ReadingSession, status_code=201)
...
@router.patch("/reading-sessions/{session_id}/end", response_model=schemas.ReadingSession)
...
def get_reading_statistics(
    novel_id: Optional[int] = Query(None, description="Get stats for specific novel only"),
    db: Session = Depends(get_db),
):
    """
...
def get_all_user_preferences(
    status: Optional[schemas.UserStatus] = Query(None, description="Filter by reading status"),
    ...
    """
    try:
        preferences = service.get_all_user_preferences(db, status, is_favorite, limit, offset)
        return preferences
    except LightNovelBookmarksException as e:
        _handle_service_exception(e) 