...
def create_slime_demo(db: Session = Depends(get_db)) -> schemas.LightNovel:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create demo novel: {str(e)}"
        )
...
def create_all_demos(db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create demo novels: {str(e)}"
        )
    ...
    return {
        ...
        "extraction_features": {
            ...
            "chapter_count": {
                ...
                "examples": [
                    "'355 Chapters (Completed)' → 355 chapters, completed status"
                ],
                "benefits": [
                    "Much faster than scraping individual chapters",
        ...
        "api_changes": {
            "removed": ["GET /api/novels/{id}/volumes", "Volume-related endpoints"],
            "added": [
...
def test_author_extraction() -> dict:
    ...
    result_author = (
        " & ".join(extracted_authors) if extracted_authors else "Unknown Author"
    )
    ...
    return {
        ...
        if len(extracted_authors) == 2 and "伏瀬" in extracted_authors
        else "❌ Issue detected",
    }
...
def test_chapter_extraction() -> dict:
    ...
    return {
        ...
        "sample_html": sample_html.strip(),
        "raw_text": status_elem.get_text(strip=True)
        if status_elem
        else "No editstatus found",
        ...
        "status": "✅ Working correctly"
        if extracted_data.get("total_chapters", 0) == 355
        and extracted_data.get("status") == "completed"
        else "❌ Issue detected",
    }
