...
class CORSSettings(BaseSettings):
    ...
    headers: List[str] = Field(default=["*"], description="Allowed CORS headers")
...
class ScraperSettings(BaseSettings):
    ...
    delay: int = Field(
        default=6, ge=1, le=30, description="Delay in seconds between scraper requests"
    )
    timeout: int = Field(
...
class AppSettings(BaseSettings):
    ...
    title: str = Field(
        default="Light Novel Bookmarks API", description="Application title"
    )
    ...
    port: int = Field(default=8000, ge=1000, le=65535, description="Application port")
