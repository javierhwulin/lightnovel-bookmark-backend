...
from app.schemas.novel import LightNovelCreate, LightNovelUpdate, ChapterCreate, ChapterUpdate
...
def update_novel(db: Session, novel_id: int, novel_data: LightNovelUpdate) -> LightNovel:
    """
...
def get_novel_chapters(db: Session, novel_id: int) -> List[Chapter]:
    ...
    try:
        ...
        return db.query(Chapter).filter(Chapter.novel_id == novel_id).order_by(Chapter.number).all()
    except SQLAlchemyError as e:
...
def update_chapter(db: Session, novel_id: int, chapter_id: int, chapter_data: ChapterUpdate) -> Chapter:
    """
    Update a chapter
