...
from app.services.novelupdates_cloudscraper import NovelUpdatesCloudScraper, NovelUpdatesScraperError
from app.models.novel import LightNovel, Chapter
from app.schemas.novel import LightNovelCreate, ChapterCreate
...
def scrape_and_create_novel(db: Session, novelupdates_url: str) -> LightNovel:
    ...
    try:
        ...
        if not novelupdates_url.startswith('http'):
            raise ValueError("Only NovelUpdates URLs are supported. Search functionality has been removed.")
        ...
        existing_novel = db.query(LightNovel).filter(
        ...
        db_novel = LightNovel(
            ...
            cover_url=scraped_data.get('cover_url', None),  # Now extracts cover images from NovelUpdates
...
def update_novel_chapters(db: Session, novel_id: int, novelupdates_url: str) -> LightNovel:
    """
    ...
    try:
        ...
        if scraped_data.get('source_url') and db_novel.source_url != scraped_data['source_url']:
...
def get_novel_info_preview(novelupdates_url: str) -> Dict:
    ...
    try:
        ...
        scraped_data = scraper.scrape_novel_by_url(novelupdates_url, scrape_chapters=False)
        
        # Format preview data
        preview = {
            'title': scraped_data.get('title', 'Unknown'),
            'author': scraped_data.get('author', 'Unknown Author'),
            'description': scraped_data.get('description', '')[:500] + '...' if len(scraped_data.get('description', '')) > 500 else scraped_data.get('description', ''),
            'status': scraped_data.get('status', 'unknown'),
