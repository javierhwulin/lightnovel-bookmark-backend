...
class NovelUpdatesScraper:
            ...
            try:
                ...
                if response.status_code == 403:
                    if attempt < retries - 1:
                        ...
                        raise NovelUpdatesScraperError(
                            f"Access denied (403) after {retries} attempts. NovelUpdates may be blocking automated requests."
                        )
    ...
    def _extract_novel_info(self, soup: BeautifulSoup) -> Dict[str, str]:
        ...
        author_selectors = ['a[href*="/nauthor/"]', ".author a", ".series-author a"]
        ...
        # Extract status
        status_selectors = [".seriesstat", ".series-status", ".status"]
        ...
        # Extract genres
        genre_selectors = ['a[href*="/genre/"]', ".genre a", ".series-genre a"]

        for selector in genre_selectors:
            genre_elements = soup.select(selector)
            if genre_elements:
                novel_info["genres"] = [
                    elem.get_text(strip=True) for elem in genre_elements
                ]
                break
    ...
    def _extract_chapters_from_table(self, soup: BeautifulSoup) -> List[ChapterInfo]:
        ...
        for row in rows:
            ...
            volume_num, chapter_num, clean_title = self._parse_chapter_title(
                chapter_title
            )
        ...
        for chapter in all_chapters:
            ...
            if vol_num not in volumes_dict:
                volumes_dict[vol_num] = VolumeInfo(
                    volume_number=vol_num, title=f"Volume {vol_num}", chapters=[]
                )
