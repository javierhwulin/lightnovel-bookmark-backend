...
def get_user_preferences(db: Session, novel_id: int) -> Optional[UserPreferences]:
    ...
    try:
        ...
        preferences = db.query(UserPreferences).filter(
            UserPreferences.novel_id == novel_id
        ).first()
        
        return preferences
        
    except SQLAlchemyError as e:
        raise DatabaseError(f"Database error while retrieving user preferences: {str(e)}")
...
def create_or_update_user_preferences(
    ...
    try:
        ...
        db.rollback()
        raise DatabaseError(f"Database error while creating/updating user preferences: {str(e)}")
...
def update_reading_status(db: Session, novel_id: int, status_data: StatusUpdate) -> UserPreferences:
    """
    ...
    try:
        ...
        if not preferences:
            ...
            preferences_create = UserPreferencesCreate(user_status=status_data.user_status)
        ...
        if status_data.user_status == UserStatus.READING and not preferences.date_started:
            preferences.date_started = date.today()
        elif status_data.user_status == UserStatus.COMPLETED and not preferences.date_completed:
...
def update_reading_progress(db: Session, novel_id: int, progress_data: ProgressUpdate) -> UserPreferences:
    """
...
def start_reading_session(db: Session, novel_id: int, session_data: ReadingSessionCreate) -> ReadingSession:
    """
...
def end_reading_session(db: Session, session_id: int, end_data: ReadingSessionEnd) -> ReadingSession:
    """
...
def get_reading_sessions(
    ...
    try:
        ...
        sessions = query.order_by(desc(ReadingSession.started_at)).offset(offset).limit(limit).all()
        return sessions
        
    except SQLAlchemyError as e:
        raise DatabaseError(f"Database error while retrieving reading sessions: {str(e)}")


def get_reading_statistics(db: Session, novel_id: Optional[int] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise DatabaseError(f"Database error while calculating reading statistics: {str(e)}")
...
def get_all_user_preferences(
    ...
    try:
        ...
        preferences = query.order_by(desc(UserPreferences.updated_at)).offset(offset).limit(limit).all()
        return preferences
