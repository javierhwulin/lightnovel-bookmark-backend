...
class LightNovelBase(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None,
        max_length=5000,
        description="Novel description or synopsis"
    )
    ...
    genres: List[str] = Field(
        ...
        examples=[["Fantasy", "Adventure", "Comedy"], ["Action", "Drama", "Supernatural"]]
    )
    source_url: Optional[str] = Field(
        default=None,
        max_length=2000,
        description="URL to the original novel source (e.g., NovelUpdates page)",
        examples=["https://www.novelupdates.com/series/tensei-shitara-slime-datta-ken/"]
    )
    ...
    total_chapters: Optional[int] = Field(
        default=0,
        ge=0,
        description="Total number of chapters (extracted from source)"
    )
    total_volumes: Optional[int] = Field(
        default=0,
        ge=0,
        description="Total number of volumes (if applicable)"
    )
...
class LightNovelUpdate(BaseModel):
    ...
    source_url: Optional[str] = Field(
        default=None,
        max_length=2000,
        description="URL to the original novel source"
    )
    ...
    raw_status: Optional[str] = Field(
        default=None,
        max_length=1000,
        description="Raw status text from the source"
    )
...
class PreviewResponse(BaseModel):
    ...
    total_chapters: int = Field(default=0, description="Total chapter count from source")
    total_volumes: int = Field(default=0, description="Total volume count from source")
    content_type: str = Field(default="unknown", description="Content organization type")
    raw_status: Optional[str] = Field(default=None, description="Raw status text from source")
