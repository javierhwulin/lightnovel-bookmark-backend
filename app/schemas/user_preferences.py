...
class UserPreferencesBase(BaseModel):
    ...
    is_favorite: bool = Field(
        default=False, description="Whether this novel is marked as favorite"
    )
    ...
    date_started: Optional[date] = Field(
        default=None, description="Date when user started reading this novel"
    )
    date_completed: Optional[date] = Field(
        default=None, description="Date when user completed this novel"
    )
...
class StatusUpdate(BaseModel):
    ...
    user_status: UserStatus = Field(..., description="New reading status for the novel")
...
class ProgressUpdate(BaseModel):
    ...
    current_chapter: Union[int, float] = Field(
        ..., ge=0, description="Current chapter number (supports decimal values)"
    )
    user_status: Optional[UserStatus] = Field(
        default=None, description="Optional status update along with progress"
    )
...
class ReadingSessionBase(BaseModel):
    ...
    chapter_number: Union[int, float] = Field(
        ..., ge=0, description="Chapter number being read (supports decimal values)"
    )
    duration_minutes: Optional[int] = Field(
        default=None, ge=0, description="Reading session duration in minutes"
    )
    device_type: Optional[DeviceType] = Field(
        default=DeviceType.WEB, description="Device used for reading"
    )
...
class ReadingSessionEnd(BaseModel):
    ...
    duration_minutes: int = Field(
        ..., ge=0, description="Total reading session duration in minutes"
    )
